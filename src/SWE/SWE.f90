...
#       if defined(_IMPI)
        type t_impi_bcast
            ...
            character(len=64)      :: s_file_stamp  ! MPI_CHARACTER
...
#           if defined(_IMPI)
            !At this poitn, JOINING ranks don't have the correct s_time
            !We don't want them to bcast wrong data or create wrong file
                ...
                !== Case pure expansion ==
                    ...
                    if (status_MPI .ne. MPI_ADAPT_STATUS_LEAVING) then
                        bcast_packet = t_impi_bcast( &
                                ...
                                grid%sections%is_forward(), swe%output%s_file_stamp)
...
#           endif
        ...
        subroutine create_impi_bcast_type(impi_bcast_type)
            ...
            !    character(len=64)      :: s_file_stamp  ! MPI_CHARACTER x64
            !end type t_impi_bcast
            !************************

#           if defined(_IMPI)
            integer :: lens(4), types(4), disps(4), type_size, err
            ...
            disps(4) = disps(3) + lens(3) * sizeof(l_sample)
            ...
            call MPI_Type_struct(4, lens, disps, types, impi_bcast_type, err); assert_eq(err, 0)
